...
if (!directoryName) {
    console.error(
        "Please specify the project directory name: vertigis-web-sdk create <project-name> [--skip-install] [--pnpm]"
    );
    process.exit(1);
}
...
if (fs.existsSync(customTemplateDir) && fs.existsSync(targetPath)) {
    ...
    if (fs.existsSync(pkgPath)) {
        try {
            ...
            console.log(
                "[ENTERPRISE] Added enterprise dependencies, auth:portal, and skill:add scripts to package.json"
            );
        } catch (e) {
