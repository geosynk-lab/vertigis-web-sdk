...
if (!directoryName) {
    console.error(
        "Please specify the project directory name: vertigis-web-sdk create <project-name> [--npm | --pnpm | --skip-install]"
    );
    process.exit(1);
}
...
if (fs.existsSync(customTemplateDir)) {
    ...
    const filesToUpdate = [path.join(targetPath, "app/layout.xml"), path.join(targetPath, "src/index.ts")];
...
if (fs.existsSync(pkgPath)) {
    try {
        ...
        pkg.devDependencies["@vertigis/web-sdk"] = "latest";
        pkg.devDependencies["@vertigis/web"] = "^5.42.0";
        ...
        console.log("[ENTERPRISE] Configured enterprise dependencies and scripts in package.json");
    } catch (e) {
...
// 8. Ensure @vertigis/web-sdk resolves in node_modules even when installed as @geosynk/vertigis-web-sdk
